...
| 头文件 | 内容 | 详细参考 |
| --- | --- | --- |
| `types.h` | `MigoResult` 与全部错误码；`MigoEngine`/`MigoSession`/`MigoSurfaceAttachment`/`MigoSurfaceRelease` 前向声明；`MIGO_API`、`MIGO_CALL` 宏；平台检测宏（`MIGO_PLATFORM_IS_ANDROID` 等）；`MIGO_C_ABI_CANDIDATE`、`MIGO_C_ABI_HAS_RUNTIME` | [types.mdx](/docs/reference/types/) |
...
| `input.h` | 触摸（`MigoTouchEvent`）、桌面指针（`MigoPointerEvent`）、滚轮（`MigoWheelEvent`）、软键盘（`MigoKeyboardEvent`）、物理按键、IME 合成、手柄；`migo_session_send_*` 系列；输入 backpressure 约定 | [input.mdx](/docs/reference/input/) |
| `external_frames.h` | 跨进程帧提交（`migo_session_submit_external_frame`）、帧请求（`migo_session_request_external_frame`）、同步屏障、资源通道 | [external-frames.mdx](/docs/reference/external-frames/) |
| `capabilities.h` | `MigoCapabilities`；`migo_query_capabilities`：查询已链接库支持的 ABI 版本范围与可附加 platform 类型；此查询在任何句柄创建之前即可调用 | [capabilities.mdx](/docs/reference/capabilities/) |
| `platform/*.h` | 各平台强类型 surface descriptor：`android.h`（`ANativeWindow*`）、`win32.h`（`HWND`）、`winui.h`（SwapChainPanel）、`x11.h`（`Display*` + `Window`）、`wayland.h`（`wl_display*` + `wl_surface*`）、`openharmony.h`（`OHNativeWindow*`）、`macos.h`、`ios.h` | [surface.mdx](/docs/reference/surface/) |
| `migo.h` | 综合头：仅 `#include` types / capabilities / surface / session / input，无额外声明；大多数宿主包含此文件即可 | [engine.mdx](/docs/reference/engine/) |
...
| 文档 | 覆盖头文件 |
| --- | --- |
| [类型与宏](/docs/reference/types/) | `types.h` |
| [Engine 与 Session](/docs/reference/engine/) | `migo.h`（umbrella） |
| [Session API](/docs/reference/session/) | `session.h` |
| [Surface 生命周期](/docs/reference/surface/) | `surface.h`、`platform/*.h` |
| [输入事件](/docs/reference/input/) | `input.h` |
| [外部帧](/docs/reference/external-frames/) | `external_frames.h` |
| [运行库能力查询](/docs/reference/capabilities/) | `capabilities.h` |
