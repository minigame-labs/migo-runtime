...
生产方的 socket 上有两种消息：帧包，和控制消息（目前只有"请求下一帧"）。传输层**问库**这条消息该进哪扇门，而不是自己比较 magic——宿主自己编码这条规则，就是 wire 格式又多一份会漂移的实现。

- `MIGO_UPLINK_MESSAGE_CONTROL`：交给 `migo_session_submit_uplink_control`。
- `MIGO_UPLINK_MESSAGE_FRAME`：交给 `migo_session_submit_external_frame`。**凡不是控制消息的都是这个**，包括连帧也不是的字节——由帧 ingress 以生产方看得到的判决拒绝它。
