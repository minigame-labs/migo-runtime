...
sidebar:
  order: 4
description: Complete C ABI reference for migo_session_* functions and their
...
A Session is a host-owned execution unit: one Session carries one piece of content, binds one Surface, and dispatches all callbacks through one dispatcher. An Engine may hold multiple Sessions concurrently; the host must serialize every call on an individual Session. See [engine.mdx](/docs/en/0.9/reference/engine/) for Engine creation and destruction, and [surface.mdx](/docs/en/0.9/reference/surface/) for Surface attach, detach, and release.
...
## migo\_session\_set\_host\_callbacks
...
## migo\_session\_set\_visibility
...
## migo\_session\_set\_network\_status
...
## migo\_session\_set\_battery\_status
