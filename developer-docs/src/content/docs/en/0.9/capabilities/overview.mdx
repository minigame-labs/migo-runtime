...
sidebar:
  order: 1
description: "The three categories of capabilities that content (migo.*)
  requests from the host: input events, soft keyboard, and Android handler
  contracts. Lists the contract owner, fallback behavior, and platform
...
slug: en/0.9/capabilities/overview
---
...
<CardGrid>
  <LinkCard title="Commerce: Ads" href="/docs/en/0.9/capabilities/commerce/ads/" description="The SDK is already delivered to content; the host calls the other end. Rewarded video settled fallback and callback dispatch are all covered here." />

  <LinkCard title="Commerce: Payment" href="/docs/en/0.9/capabilities/commerce/payment/" description="Midas/Android payment handler contract and fallback behavior when isMidasPaymentSupported defaults to false." />

  <LinkCard title="Platform Services" href="/docs/en/0.9/capabilities/services/auth/" description="Login/auth, share, settings, permissions, subpackages, logging, mini-program navigation — contracts and fallbacks for seven platform handlers." />

  <LinkCard title="Soft Keyboard and Key Events" href="/docs/en/0.9/capabilities/keyboard/" description="The only C-ABI-level capability: the on_show/hide/update_keyboard triad plus IME composition events." />
</CardGrid>
...
Full structs and flags are on the [Input Events](/docs/en/0.9/reference/input/) page; that page is the data-plane reference. This page only answers "who is responsible for sending what to whom".
...
The host reports events back via `migo_session_send_keyboard_event` — four types: INPUT / CONFIRM / COMPLETE / HEIGHT\_CHANGE. Height events must be in **CSS px** (for the cost of getting the DPI conversion wrong, see the [Soft Keyboard page](/docs/en/0.9/capabilities/keyboard/)).

See [Soft Keyboard and Key Events](/docs/en/0.9/capabilities/keyboard/) for details.
...
Details: [Ads](/docs/en/0.9/capabilities/commerce/ads/) / [Payment](/docs/en/0.9/capabilities/commerce/payment/).
...
The fields and ABI-check protocol for runtime capability queries are covered in [Runtime Capability Queries](/docs/en/0.9/reference/capabilities/).
