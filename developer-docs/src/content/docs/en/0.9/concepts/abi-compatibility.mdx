...
sidebar:
  order: 6
description: The current status (CANDIDATE) of C ABI compatibility discipline,
  existing forward and backward compatibility mechanisms, validation lanes, and
  the remaining steps to promote it to stable v1.
...
slug: en/0.9/concepts/abi-compatibility
---
...
Once all three are cleared, the promotion PR will do all of the following together: flip `MIGO_C_ABI_CANDIDATE` to 0, remove the candidate notice pinned at the top of the [API Reference Overview](/docs/en/0.9/reference/overview/), and rewrite this page's status section as "frozen." Until then, betting on the ABI as a stable release is at your own risk.

## Related

* [API Reference Overview](/docs/en/0.9/reference/overview/) — the CANDIDATE notice and eight-page mapping
* [Threading and Dispatch](/docs/en/0.9/concepts/threading-and-dispatch/) — the concurrency contract that must be frozen alongside it
