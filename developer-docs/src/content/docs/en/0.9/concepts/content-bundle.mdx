...
sidebar:
  order: 5
description: What a mini-game bundle is, where it lives in the engine file tree,
  the isolation rules for content_id, and who is responsible for staging,
...
slug: en/0.9/concepts/content-bundle
---
...
3. **Never guess the path.** The directory is always given to you by the engine's API (Android: `session.getPaths().getCodeDir()`; on the C ABI side see the path fields in [Session](/docs/en/0.9/reference/session/)). Hard-coding internal paths makes your host brittle to any future engine layout change.
...
| Update | Host + engine contract | New content means a new session: after updating the bundle, start a fresh Session (see [Session Lifecycle](/docs/en/0.9/concepts/lifecycle/)). Do not expect to hot-swap content by reusing a session |
...
* [Session Lifecycle](/docs/en/0.9/concepts/lifecycle/) — how sessions and content relate to each other
* [Download and Verification](/docs/en/0.9/release/download-verification/) — signature verification for SDK artifacts themselves
* [Android Quick Start §5–§6](/docs/en/0.9/getting-started/android/) — platform-specific staging walkthrough
