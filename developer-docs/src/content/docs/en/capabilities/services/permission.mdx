...
sidebar:
  order: 2
description: Android PermissionHandler contract — the request-response model of requestScope, the three channels of PermissionSink (setScope/resolveRequest/failRequest), and the design rationale for deny-all behavior when no handler is registered.
