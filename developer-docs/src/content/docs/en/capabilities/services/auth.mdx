...
sidebar:
  order: 1
description: Android AuthHandler contract — the two required methods login/checkSession, default fallbacks for getUserInfo/getPhoneNumber, and the "no auth handler" semantics when no handler is registered.
