...
sidebar:
  order: 6
description: Android NavigationHandler contract — the dual response model for mini-program navigation and customer-service entry, request fields, and the two "not supported" fallbacks when no handler is registered.
