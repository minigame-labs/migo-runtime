...
The subset actually supported by the current build is reported by `migo_query_capabilities` in `platform_kinds` (see [capabilities.mdx](/docs/en/reference/capabilities/)). Passing a defined `platform_kind` that this build does not implement returns `MIGO_ERROR_UNSUPPORTED_PLATFORM`; passing a completely undefined value returns `MIGO_ERROR_INVALID_ARGUMENT`.
