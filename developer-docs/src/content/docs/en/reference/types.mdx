...
sidebar:
  order: 2
description: Export macros defined in types.h, ABI candidate status, platform detection, error codes, public types, and cross-platform linking conventions.
...
Sessions must be destroyed before their Engine; a Surface attachment must complete the detach/release flow before its Session can be destroyed. See [engine.mdx](/docs/en/reference/engine/), [session.mdx](/docs/en/reference/session/), and [surface.mdx](/docs/en/reference/surface/).
