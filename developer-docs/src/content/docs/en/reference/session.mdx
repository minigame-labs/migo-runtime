...
A Session is a host-owned execution unit: one Session carries one piece of content, binds one Surface, and dispatches all callbacks through one dispatcher. An Engine may hold multiple Sessions concurrently; the host must serialize every call on an individual Session. See [engine.mdx](/docs/reference/engine/) for Engine creation and destruction, and [surface.mdx](/docs/reference/surface/) for Surface attach, detach, and release.
