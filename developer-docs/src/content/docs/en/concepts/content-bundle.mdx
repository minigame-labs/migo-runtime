...
3. **Never guess the path.** The directory is always given to you by the engine's API (Android: `session.getPaths().getCodeDir()`; on the C ABI side see the path fields in [Session](/docs/reference/session/)). Hard-coding internal paths makes your host brittle to any future engine layout change.
...
| Update | Host + engine contract | New content means a new session: after updating the bundle, start a fresh Session (see [Session Lifecycle](/docs/concepts/lifecycle/)). Do not expect to hot-swap content by reusing a session |
...
- [Session Lifecycle](/docs/concepts/lifecycle/) — how sessions and content relate to each other
- [Download and Verification](/docs/release/download-verification/) — signature verification for SDK artifacts themselves
- [Android Quick Start §5–§6](/docs/getting-started/android/) — platform-specific staging walkthrough
