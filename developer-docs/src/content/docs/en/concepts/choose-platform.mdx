...
- **Production today** → Android, no second choice; the [Android quickstart](/docs/getting-started/android/) is the primary path.
- **HarmonyOS listing** → the OpenHarmony project genuinely runs: clone the repo, open DevEco, follow the [OpenHarmony quickstart](/docs/getting-started/openharmony/); but before buying hardware, verify multi-finger behavior on the API20 emulator — real-device multi-touch regression is your responsibility.
- **Desktop channel (Windows)** → the published DLL integrates the C ABI directly; write your own Win32 window and input pump; the [Windows quickstart](/docs/getting-started/windows/) starts from the published package and the API surface pinned by `test-windows-sdk-contract.sh`.
- **Apple ecosystem** → set expectations against the current integration surface, not a polished quickstart; the [Apple doc](/docs/getting-started/apple/) lists the **actually existing** SwiftPM lanes and their gaps, without fabricating an App Store-ready flow.
