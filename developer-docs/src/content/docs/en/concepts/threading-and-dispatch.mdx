...
sidebar:
  order: 4
description: The C ABI threading contract — which handles the host serializes, how to write a dispatcher, which thread callbacks run on, and why destroy is the final thread rendezvous point.
...
After reading this page, if you want to walk through the state machine, continue with the destroy section of [Session Lifecycle](/docs/en/concepts/lifecycle/); for function-level C ABI signatures, see the [Session](/docs/en/reference/session/) and [Surface](/docs/en/reference/surface/) reference pages.
