...
sidebar:
  order: 1
description: 在 Android 上把 ANativeWindow 接到引擎 Surface：描述符构造、generation 计数器陷阱、纯 C 宿主的 JNI 初始化时机。
...
本页只覆盖 Android 平台的接入增量。Attach / detach / release 的通用合同（generation 语义、错误码、`MigoSurfaceDescriptor` 字段规则）见 [Surface 接口](/docs/reference/surface/)；Session 生命周期见 [生命周期概念](/docs/concepts/lifecycle/)。
