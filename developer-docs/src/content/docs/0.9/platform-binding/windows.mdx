...
slug: 0.9/platform-binding/windows
---

本页只写 Windows 原生 surface 的平台差异。通用 attach、resize、异步 detach 和 release 规则见 [Surface 接口](/docs/0.9/reference/surface/)；Session 的关闭顺序见 [生命周期概念](/docs/0.9/concepts/lifecycle/)。

**可用性：** 0.9 已发布 Win32 C ABI（`migo.dll` / `migo.lib`，x86\_64 与 arm64）；`migo_query_capabilities` 会报告 `MIGO_PLATFORM_WIN32_HWND`。WinUI `SwapChainPanel` 仍是未来实现，不能按可用平台接入。
