//! Inline image sources: `data:` URLs, and the bytes an `http(s)://` source
//! fetched.
//!
//! These bypass the filesystem VFS and feed decoded RGBA directly into the GPU
//! upload pipeline. Fetching an `http(s)://` source is the caller's -- it runs
//! under that execution's network policy -- and what comes back is decoded
//! here, under the same budgets as a data URL.
